...
void echo(int connfd)
{
    ...
    rio_t rio;
    int imgfd;
    imgfd=Open("out.png",O_WRONLY|O_CREAT,mode);
    Rio_readinitb(&rio, connfd);
    while((n = Rio_readlineb(&rio, buf, MAXLINE)) != 0) {
        //printf("server received %d bytes\n", n);
        Rio_writen(imgfd, buf, n);
    }
    Close(imgfd);
    if (fork()==0)
    {
        execl("/home/prkmufc/EOS_assignments/Project/Actual_Project/c2grey","./c2grey","out.png",(char *)NULL);
    }
    Wait(NULL);
}
